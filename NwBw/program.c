...
int main(int argc, char** argv) {
    ...
    if (argc < 0){
        ...
        SIZE_TO_SEND = atoi(argv[1]);
    }

    int sockfd,sockfd2;
    char tosend[SIZE_TO_SEND]; //a char (1byte) to send to receivers
    //char tosend = 's'; //a char (1byte) to send to receivers
    char ack[SIZE_TO_SEND];
    //char ack;
...
//	clock_gettime(CLOCK_MONOTONIC, &tstart);
        ...
        optval=recv(sockfd2,&ack,sizeof(char)* SIZE_TO_SEND,0);
